...
function toggleCollapse(element) {
    ...
    allCollapsible.forEach(collapsible => {
        if (collapsible !== element && collapsible.parentElement.classList.contains('active')) {
            collapsible.parentElement.classList.remove('active');
            collapsible.nextElementSibling.style.display = 'none';
            collapsible.querySelector('.collapsible-symbol').textContent = '+';
        }
    });
...
function toggleSubmenu(navItem) {
    const submenu = navItem.querySelector('.submenuu');
    const allNavItems = document.querySelectorAll('.nav_item');

    allNavItems.forEach(item => {
        if (item !== navItem && item.classList.contains('expanded')) {
            item.classList.remove('expanded');
            item.querySelector('.submenuu').style.display = 'none';
        }
    });
